...
// AI-powered "Recommended For You" section (Claude). Renders only when signed in
...
const AIRecommendations = () => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setLoading(true)
            try {
                const { data } = await axios.post(backendUrl + '/api/ai/recommendations', {}, { headers: { token } })
    ...
    if (!token || (!loading && products.length === 0)) return null

    return (
        <div className='my-16'>
            <div className='flex items-center gap-2 mb-8'>
                <Sparkles className='w-6 h-6 text-locoxo-orange' />
                <h2 className='text-2xl sm:text-3xl font-heading font-extrabold text-locoxo-header'>RECOMMENDED FOR YOU</h2>
            </div>
            {loading ? (
